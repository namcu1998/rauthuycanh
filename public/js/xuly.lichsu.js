...
function xulyData(getid, array){
	var html = array.map(function(x){
		return '<tr>' + '<td>' + x.light + "lx" + '</td>' + '<td>' + x.nhietdo + '*C' + '</td>' +'<td>' + x.doam + '%' +  '</td>' + '<td>' + x.thoigian + '</td>' + '<td>' + convert(x.fanHumi) + '</td>' + '<td>' + convert(x.speaker) + '</td>' + '<td>' + convert(x.fanTemp) + '</td>' + '<td>' + convert(x.fan) + '</td>' + '</tr>';
	});
